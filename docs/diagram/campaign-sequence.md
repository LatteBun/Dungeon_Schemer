...
sequenceDiagram
    ...
    opt 승급 조건 충족
        Player->>UI: 상단 길잡이 등급 버튼
        UI->>Flow: 승급 선택 오버레이 열기
        Player->>UI: 명성 또는 골드 경로 선택
        Flow->>Rules: 한 단계 승급 검증·확정
        Rules-->>State: 등급 변경·공고 무효화
        Flow->>Rules: 같은 seed·worldTurn으로 공고 재생성
        State-->>UI: 승급 결과·갱신된 공고
    end
    Player->>UI: 공고 선택
    UI->>Flow: 계약 요청
...
- 승급은 자동으로 일어나지 않는다. 다음 게시판 상단의 길잡이 등급 버튼을 눌러야 오르고 강등은 없다.
- 승급은 게시판에서만 명성 또는 골드 중 하나를 선택해 한 단계 실행한다. 확정 뒤 같은 시드·월드턴으로 공고를 갱신한다.
