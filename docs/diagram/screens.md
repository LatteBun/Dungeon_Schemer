...
인트로부터 캠페인 엔딩까지의 화면 일곱 장이다. **구현된 화면을 1920×1080
고정 캔버스에서 그대로 캡처한 것**이며, 손으로 그린 와이어프레임이 아니다.
...
- 승급 요건을 만족하면 명성 승급과 골드 승급을 따로 고른다. 두 경로를 하나로 합치지 않는다.
