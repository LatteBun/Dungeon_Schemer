...
2. [탐험 시퀀스](expedition-sequence.md): 경로, 정보 전달, 사건과 자동 보스전
3. [캠페인 상태 전이](campaign-state.md): 정산과 네 엔딩의 판정 우선순위
4. [탐험 상태 전이](expedition-state.md): 공식 단계와 조건부·조기 전이
5. [대표 화면 와이어프레임](screen-wireframes.md): 핵심 화면군 5개의 정보 배치

## 한눈에 보기

[![대표 화면 5개 전체 모음](png/screen-overview.png)](svg/screen-overview.svg)
