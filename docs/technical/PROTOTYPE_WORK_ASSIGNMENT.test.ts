 ...
 * 배정표 무결성 검사.
 *
 * 배정표의 규약은 문서의 「관리 원칙」에 산문으로 적혀 있다. 사람이
...
/**
 * 7열 배정표의 행만 골라낸다. 문서에는 「진행 순서와 3트랙」의 2열·3열
 * 표도 있으므로 열 개수와 첫 칸의 ID 모양으로 걸러낸다.
 */
...
/** 그래프에서 이 작업을 막는 작업들. */
function incoming(id: string): string[] {
  return [...graph.edges]
...
describe("표와 그래프가 같은 구조를 담는다", () => {
  ...
  it("`풀리는 것`이 그래프의 나가는 간선과 같다", () => {
    const mismatched = rows
      .filter(
        (row) =>
          sorted(row.unblocks).join(" ") !== sorted(outgoing(row.id)).join(" "),
      )
      .map(
        (row) =>
