...
현재 목표는 로그인 없이 브라우저에서 던전 15개 캠페인 루프를 실행하고 검증하는 것이다.

- Supabase 연동, 저장·복원, 사용자 로그인, 비밀 환경 변수, 운영 도메인은 현재 범위에 넣지 않는다.
- 개발 중 기능 확인은 각자의 Codespaces에서 수행한다.
- 통합된 데모 확인은 Vercel Hobby에 배포된 `main`으로 수행한다.
- 새 캠페인 구현은 승인된 상위 spec과 상세 plan이 모두 준비된 뒤 시작한다.
