...
현재 모든 action ID는 `⬜ 대기`다. 구현을 시작하면 상세 plan의 Task를 체크하고, 해당 action의 완료 기준을 모두 충족했을 때만 action을 `✅ 완료`로 바꾼다. Q2는 승인된 spec의 이번 구현 범위가 배포 전까지이므로 별도 후속 작업으로 남긴다.

## 배정표

| ID | 구현 영역 | 완료 기준 | 선행 | 풀리는 것 | 담당 | 상태 |
| --- | --- | --- | --- | --- | --- | --- |
| F1 | 캠페인 도메인·상태 계약 | 캠페인·탐험 상태와 모든 새 규칙 입력·출력 타입, 시드 스트림 이름이 있고 타입 검사 통과 | — | **C1 C2 E1 U1** |  | ⬜ |
| C1 | 캠페인 초기화·게시판 | 15개 던전 생성, 정렬, 최대 5개 공고와 시드 기반 파티 연결·명성 제한 테스트 통과 | **F1** | **C3 U1** |  | ⬜ |
| C2 | 지속 파티 | 충원, 최대 파티 자동 재편, 개인 상태 유지, 비출전 5% 회복 테스트 통과 | **F1** | **C3** |  | ⬜ |
...
| E1 | 등급별 지도·정보 기회 생성 | 7/9/11/13 지점 대칭 지도와 모든 경로의 C2/B3/A4/S5 정보 횟수 테스트 통과 | **F1** | **E2 U2** |  | ⬜ |
...
| U1 | 게시판·캠페인 HUD | 공고·지원 조건·파티 상태와 등급·명성·두 골드·점수·남은 던전 표시 | **F1 C1** | **I1** |  | ⬜ |
