...
구현 중 발견했지만 규칙을 임의로 고치지 않고 남긴 것들이다. 상위 plan이 `수치가 목표와 다르면 코드를 임의로 맞추지 말고 보고서를 먼저 남긴 뒤 공식 밸런스 상수 변경을 별도 커밋으로 수행한다`고 정했으므로 `C4` 보고서가 먼저다.

**명성 음수 절벽 (C3에서 발견).** [성장과 엔딩](../systems/PROGRESSION_AND_ENDINGS.md)이 현재 명성의 최솟값을 `제한 없음`으로 정했다. 그런데 모든 던전의 지원 최소 명성이 C급 0 이상이므로, 명성이 음수가 되는 순간 모든 공고가 잠기고 `길잡이 자격 박탈` 엔딩이 성립한다. 명성 0에서 C급 하나만 전멸해도 -10이 되므로 **명성을 미리 벌어 두지 않았다면 첫 전멸이 곧 캠페인 종료**다. 전멸로 골드를 버는 배신 전략이 초반에 성립하지 않는다는 뜻이라 백테스트의 `전멸 골드 우선` 전략이 시작하자마자 끝날 것으로 예상한다. 조정 후보는 명성 하한 0, C급 지원 명성을 음수로 두기, 전멸 명성 손실 축소 셋이다.

**보스전 전 HP 편차 (E3에서 발견).** S급 경로는 일반 사건 7개 중 3칸이 시드로 정해져 갈래마다 구성이 다르다. 전원 관망일 때 몬스터가 몰린 갈래는 보스방 도착 전에 전멸하고 상인이 몰린 갈래는 보스 뒤 HP 20으로 생존한다. 지도가 입장 전에 분류를 공개하므로 의도한 선택지이지만, 편차가 지나친지는 보고서로 확인한다.

**정보 카드 노출 빈도 (F3에서 발견).** 한 캠페인의 정보 기회는 46회이고 카드는 36장이다. 장당 평균 노출이 4회 아래인지, 특정 조합이 과도하게 반복되는지 보고한다.
...
| C4 | 캠페인 백테스트 | 기준 승급 시나리오와 10,000개 시드 전략 보고서 생성, 생성 불능 0건, 아래 밸런스 관찰 항목 보고 | — | **I1** |  | ⬜ |
