...
| F2 | Task 5~7의 콘텐츠 부분 | 사건·정보 카드·아이템 데이터를 등급별 요구량까지 채우기 |
...
F1은 도메인·탐험 계약, 구조화 오류, 시드 스트림 이름과 타입 검증을 완료해 `✅ 완료`다. 나머지 action ID는 `⬜ 대기`이며, 각 상세 plan Task의 완료 기준을 모두 충족했을 때만 갱신한다. Q2는 승인된 spec의 이번 구현 범위가 배포 전까지이므로 별도 후속 작업으로 남긴다.
...
현재 `lib/content/events.ts`의 사건은 8개이므로 A·S급 던전을 만들 수 없고, 그 상태로는 `C4`의 완료 기준인 `생성 불능 0건`을 원리적으로 통과할 수 없다. 콘텐츠 확보를 규칙 작업 안에 숨겨 두면 `E1`~`E3`이 fixture로 `✅`가 된 뒤 `C4`에서야 막히므로 별도 ID로 분리한다.

규칙 함수는 콘텐츠 풀을 인자로 받아 fixture로 검증할 수 있다. 따라서 `F2`는 `E1`~`E3`의 시작을 막지 않고 `C4`만 막는다.

사건마다 선택지 2개 이상을 완료 기준에 넣은 이유는 선택지가 하나뿐이면 확인 버튼과 같아 [핵심 게임 루프](../design/CORE_GAME_LOOP.md)의 `모든 주요 단계에서 최소 하나의 의미 있는 플레이어 결정` 제약을 만족하지 못하기 때문이다. 현재 사건 9개는 모두 선택지가 1개다. 선택지는 용사 지원·던전 지원·관망 축에서 서로 충돌하도록 고른다.
...
| F2 | 사건·카드·아이템 콘텐츠 | S급 12개를 채우는 사건 풀, 보스 주제를 포함한 진실·거짓·중립 카드 풀, 아이템·보스 피해 데이터로 네 등급이 모두 생성되고 사건마다 선택지가 2개 이상인 테스트 통과 | — | **C4** |  | ⬜ |
| C1 | 캠페인 초기화·게시판 | 15개 던전 생성, 정렬, 최대 5개 공고와 시드 기반 파티 연결·명성 제한 테스트 통과 | — | **C3 U1** |  | ⬜ |
| C2 | 지속 파티 | 충원, 최대 파티 자동 재편, 개인 상태 유지, 비출전 5% 회복 테스트 통과 | — | **C3** |  | ⬜ |
| C3 | 정산·승급·엔딩 | 생존 보상, 전멸 유품·던전 상승, 승급식, 엔딩 우선순위 테스트 통과 | **C1 C2 E3** | **C4 U3** |  | ⬜ |
| C4 | 캠페인 백테스트 | 기준 승급 시나리오와 10,000개 시드 전략 보고서 생성, 생성 불능 0건 | **C3 F2** | **I1** |  | ⬜ |
...
| U1 | 게시판·캠페인 HUD | 공고·지원 조건·파티 상태와 등급·명성·두 골드·점수·남은 던전 표시 | **C1** | **I1** |  | ⬜ |
| U2 | 지도·정보·사건 화면 | 공개 지도, 정보 전달 기회, 개인 반응과 별도 사건 행동 흐름 동작 | **E1** | **I1** |  | ⬜ |
| U3 | 정산·엔딩 화면 | 생존부터 승급까지 원인 순서와 네 엔딩·최종 등급 표시 | **C3** | **I1** |  | ⬜ |
| I1 | 캠페인 전체 통합 | 게시판→탐험→정산→다음 공고/엔딩 흐름이 시드로 재현되고 전체 검증 통과 | **C4 U1 U2 U3** | **Q1** |  | ⬜ |
