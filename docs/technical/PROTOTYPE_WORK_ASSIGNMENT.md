...
| F3 | Task 6 실행 중 발견한 콘텐츠 공백 | 보스 주제 중립 카드를 추가해 `-10%` 보정을 살리기 |
...
`E2` 구현 중에 발견한 콘텐츠 공백이다. F2가 채운 정보 카드 12장 중 주제가 `boss`인 것은 `card-truth-boss`와 `card-lie-boss` 둘뿐이고 **중립 보스 카드가 없다**. 상위 spec은 수용한 보스 정보의 피해 보정을 진실 -20%, 중립 -10%, 거짓 +25%로 정했는데, 중립 보스 카드가 없으므로 `-10%`가 실제 플레이에서 한 번도 발생하지 않는다.

보스 보장 지점은 보스 주제 카드만 제시하므로 그 지점의 선택지도 진실·거짓 두 장으로 줄어든다. 세 유형 중 고른다는 상위 spec의 문구와 어긋난다.

규칙은 중립 보스 카드가 생기면 그대로 동작하도록 이미 구현했으므로 `E3`을 막지 않는다. 막는 것은 `C4`다. 백테스트가 산출할 보정 분포에 규칙 하나가 통째로 빠진 채로 밸런스 자료를 만들게 되기 때문이다. `F2`가 `C4`만 막았던 것과 같은 이유다.

카드를 추가하지 않고 `card-neutral-route`를 `card-neutral-boss`로 **교체해서** 해결했다. `validateContentPools`가 카드 12장과 진위별 4장을 요구하므로 13장이 되면 어느 한 진위가 5장이 되어 검사를 깨뜨린다. 균형을 지키며 늘리려면 15장이 되어야 하는데, 이 작업은 죽어 있는 규칙 하나를 살리는 것이지 카드 풀을 키우는 결정이 아니다. `route` 주제가 세 장으로 가장 많아 거기서 빼면 주제 분포가 가장 고르게 남는다.
...
| F3 | 중립 보스 정보 카드 | 주제가 boss인 중립 카드가 있어 보장 지점이 세 유형을 모두 제시하고 `수용한 중립 -10%` 보정이 실제 콘텐츠에서 발생 | — | **C4** | sbh3821 | ✅ |
...
| C3 | 정산·승급·엔딩 | 생존 보상, 전멸 유품·던전 상승, 승급식, 엔딩 우선순위 테스트 통과 | — | **C4 U3** |  | ⬜ |
...
| U1 | 게시판·캠페인 HUD | 공고·지원 조건·파티 상태와 등급·명성·두 골드·점수·남은 던전 표시 | — | **I1** |  | ⬜ |
| U2 | 지도·정보·사건 화면 | 공개 지도, 정보 전달 기회, 개인 반응과 별도 사건 행동 흐름 동작 | — | **I1** |  | ⬜ |
