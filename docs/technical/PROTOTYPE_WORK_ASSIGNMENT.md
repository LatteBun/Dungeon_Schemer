...
| F3 | 중립 보스 정보 카드 | 주제가 boss인 중립 카드가 있어 보장 지점이 세 유형을 모두 제시하고 `수용한 중립 -10%` 보정이 실제 콘텐츠에서 발생 | — | **C4** |  | ⬜ |
...
| C4 | 캠페인 백테스트 | 기준 승급 시나리오와 10,000개 시드 전략 보고서 생성, 생성 불능 0건 | **C3 F3** | **I1** |  | ⬜ |
...
| E3 | 사건·자동 보스전 | 사건 행동과 보스 피해 보정·상한·사후 검증, 생존·전멸 결과 테스트 통과 | — | **C3** |  | ⬜ |
