...
### 불규칙 층 구조 지도 (2026-08-18 회의)

지도를 입구에서 한 번 갈라져 끝에서 한 번 합치는 구조에서 **깊이가 일정한 층 그래프**로 바꿨다. 갈래가 여러 번 갈라졌다 다시 합쳐지고, 층 너비가 시드마다 변해 불규칙해 보인다. 간선이 깊이를 정확히 1씩 늘리므로 어느 길로 가도 진행 횟수가 같다.
...
경로를 짧게 하고 지점을 늘려 지도가 위아래로 길어지지 않게 했다. 평균 층 너비가 1.8대라 갈래가 더 자주 갈라졌다 합쳐진다. 등급 단계가 하나 더 늘면 그 등급이 경로 8칸을 쓴다.

한 지점의 다음 선택지는 최대 2개, 한 층의 지점은 최대 3개다. 경로마다 네 분류를 지나는 규칙은 **보장 깊이 4개**로 지킨다. 그 깊이의 모든 지점이 같은 분류를 갖고, 어느 길로 가도 그 깊이를 지나므로 경로를 전수 검사하지 않는다. 나머지 깊이는 지점마다 분류가 달라 갈래별 편차가 남는다.

같은 회의에서 **상인 조우를 특수 사건으로 합쳐** 사건 분류를 네 종에서 세 종으로 줄였다. 지도는 범례 대신 지점마다 아이콘을 그린다. 몬스터·모닥불·`?`·등급별 보스 실루엣이다.

분류를 합치면서 상인 주제 조언이 빠지지 않도록 사건 주제를 콘텐츠에 연결했다. 사건은 상황 묘사와 조언 3개를 함께 제시하며, 특수 사건의 기본 피해는 상인이 0이었으므로 -12에서 **0**으로 낮췄다.

경로가 길어져 서로 다른 사건이 더 필요하므로 사건 풀을 12개에서 **20개** 로 늘렸다. 자세한 근거는 [별도 spec](../superpowers/specs/2026-08-18-sbh3821-irregular-map-generation-design.md)에 있다.
...
| 트랙 | 책임 | 주요 산출물 |
| --- | --- | --- |
| 캠페인 규칙 | 게시판, 지속 파티, 정산, 승급, 엔딩, 백테스트, 공고·계약 위험 요약, 누적 통계 | UI 없는 순수 규칙과 시뮬레이터 |
| 탐험 규칙 | 등급별 지도, 조언 기회, 사건, 자동 보스전 | 한 탐험의 재현 가능한 결과 |
| 화면 | 게시판·HUD, 조언 선택·사건 결과, 정산·엔딩 | 규칙 결과를 설명하는 사용자 흐름 |

`C3`는 탐험 규칙의 최종 결과인 `E3`를 기다린다. 화면 트랙은 목 계약으로 병렬 진행하되 `I1`에서 실제 규칙과 합친다.

### 통합과 마감

`I1`에서 게시판부터 엔딩까지 한 캠페인을 연결한다. `U4`는 그 화면들을 대표 화면 와이어프레임의 정보 구조에 맞춘다. `Q1`은 접근성, 원인 피드백, 전체 테스트와 브라우저 흐름을 검증하고 `Q2`가 통합 데모를 배포한다.

`Q1`이 `U4`를 기다리는 이유는 화면 구조가 바뀌면 접근성 점검을 두 번 하게 되기 때문이다. `B1` 밸런스 조정과 `C5`·`C6`은 화면 흐름과 독립이므로 언제든 병렬로 시작할 수 있다.
...
| F2 | Task 5~7 중 콘텐츠 계약·불변식 부분 | 사건·조언·아이템·보스 데이터를 등급별 요구량까지 채우기 |
| F3 | Task 6 실행 중 발견한 콘텐츠 공백 | 조언 풀을 주제·유형 조합마다 2개로 넓히고 지점 분류에 연동하기 |
...
| E2 | Task 5~6 | 모든 일반 지점의 조언 기회, 보스 관련 조언 보장, 개인별 조언 판정 |
| E3 | Task 6~7 | 조언 결과와 자동 보스전·사후 검증 |
| U1 | Task 9 | 게시판, 계약 확인, 캠페인 HUD |
| U2 | Task 9 | 지도, 조언 선택과 사건 결과 화면 |
| U3 | Task 9 | 정산 timeline, 최종 등급, 엔딩 화면 |
| U4 | 현재 plan 범위 밖 | 대표 화면 와이어프레임과 구현의 정보 구조를 맞춘다. [별도 spec](../superpowers/specs/2026-08-16-lattebun-wireframe-screen-fidelity-design.md) |
...
| C6 | 별도 plan Task 1~9 | 조언 유형별 전달·반응, 생환·전멸 원정, 전환점과 원정 연대기를 정산이 누적한다. [별도 spec](../superpowers/specs/2026-08-16-sanghwan-yoo-campaign-statistics-design.md) |
| B1 | Task 10 이후 | 첫 백테스트가 드러낸 세 문제의 상수 조정과 재측정. 아래 「밸런스 조정과 재측정」 절차를 따른다 |
...
C6은 조언 유형별 전달·반응, 생환·전멸 원정, 전환점, 원정 연대기와 정산 원인 사슬을 완료했다. `U4`가 함께 넘긴 파티원 소지 아이템은 사건 결과 규칙을 바꿔야 하고 백테스트 수치가 움직이므로 `B1`과 함께 볼 후속으로 남긴다.
...
`E2` 구현 중 중립 보스 조언이 없어 `수용한 중립 -10%` 보정이 실행되지 않는 것을 발견해 만든 항목이다. 현재 계약에서는 모든 일반 지점이 조언 기회이며, 지연형 보스 조언만 위험도에 따라 1회 또는 2회를 보장한다.

조언 풀은 도움·방해·중립을 각각 담아 상황 묘사와 함께 세 개를 제시한다. 활성 규칙마다 각 유형이 2개 이상 있어야 하며, 지점의 사건 분류와 단서가 조언의 주제와 정합하도록 한다.
...
**4. 조언 노출은 전략과 경로에 따라 달라진다.** 플레이어가 유형을 선택 전에 볼 수 없으므로, 노출 통계는 정답 선호가 아니라 상황 묘사·활성 규칙·경로에 따른 선택을 기록해야 한다.
...
| `lib/rules/info.ts` | `BOSS_DAMAGE_MODIFIERS` | 유형별 보스 피해 보정 |
...
| F2 | 사건·조언·아이템 콘텐츠 | 일반 사건과 보스 사건·조언·아이템·C/B/A/S 보스 데이터의 계약·불변식 테스트와 F1 연동 브라우저 검증 통과. 실제 효과 계산은 Task 6~7에서 별도 구현 | — | **C4** | SangHwan Yoo | ✅ |
| F3 | 조언 확장·지점 연동 | 활성 규칙마다 도움·방해·중립 조언이 각각 2개 이상이고, 지점의 사건 분류가 조언 주제를 정하며, `수용한 중립 -10%` 보정이 실제 콘텐츠에서 발생 | — | **C4** | sbh3821 | ✅ |
...
| C3 | 정산·승급·엔딩 | 생존 보상, 전멸 유품·던전 상승, 승급식, 엔딩 우선순위 테스트 통과 | — | **C4 C6 U3** | sbh3821 | ✅ |
| C4 | 캠페인 전이 함수·백테스트 | 게시판부터 엔딩까지 `transitionCampaign`으로 진행, 기준 승급 시나리오와 10,000개 시드 전략 보고서 생성, 생성 불능 0건 | — | **I1** | sbh3821 | ✅ |
| C5 | 공고·계약 위험 요약 | 게시판의 공고마다 사건 분류별 위험 요약이 나오고, 계약 확인에서 분류별 개수를 보여준다. 지도 생성 시드가 탐험과 일치 | — | — | SangHwan Yoo | ✅ |
| C6 | 캠페인 누적 통계 | 조언 유형별 제시·적발 횟수, 생존·전멸 파티 수, 가장 큰 전환점이 `CampaignState`에 쌓이고 엔딩 화면이 그대로 표시 | — | — | SangHwan Yoo | ✅ |
| E1 | 등급별 지도·조언 기회 생성 | 7/9/11/13 지점 대칭 지도와 모든 일반 지점의 조언 기회 테스트 통과 | — | **C5 E2 U2** | sbh3821 | ✅ |
| E2 | 용사 대상 조언 판정 | 보스 수신 제거, 개인별 반응, 보스 조언 1/1/2/2 보장과 기록 테스트 통과 | — | **E3** | sbh3821 | ✅ |
| E3 | 사건·자동 보스전 | 조언 결과와 보스 피해 보정·상한·사후 검증, 생존·전멸 결과 테스트 통과 | — | **C3** | sbh3821 | ✅ |
| U1 | 게시판·캠페인 HUD | 공고·지원 조건·파티 상태와 등급·명성·두 골드·점수·남은 던전 표시 | — | **I1** | LatteBun | ✅ |
| U2 | 지도·조언·사건 결과 화면 | 공개 지도, 조언 기회, 개인 반응과 사건 결과 흐름 동작 | — | **I1** | LatteBun | ✅ |
