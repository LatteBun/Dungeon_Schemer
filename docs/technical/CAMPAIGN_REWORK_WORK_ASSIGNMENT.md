...
문서 계층과 기존 도메인 계약이 끝났다. 새 조언·사건 계약은 `D9` 문서 개정과 `F1-2` 타입 개정에서 시작한다. 기존 `F1`·`F2-1`·`F2-2`가 끝나 `C1`과 `E1`의 선행도 풀렸다.

지금 시작 가능한 것은 대표 화면 이미지 작업(`D8`), 조언·사건 문서(`D9`), 아이템 콘텐츠(`F5`), 캠페인 초기화(`C1`), 위험도별 지도(`E1`), 인트로 화면(`U2`)이다. 여섯 개가 서로 파일이 겹치지 않는다. `F3-1`~`F3-3`은 더 이상 시작 가능한 작업이 아니다.
