...
지금 **40개가 끝났고 3개가 남았다.** 남은 것은 밸런스·검증·배포다.
...
| U6 | 정산·엔딩 화면 | 정산 원인 순서, 위험도·보상 변화와 승급 가능 여부, 엔딩 5종 판정 원인·누적 통계 표시. 승급 버튼·선택·결과는 제공하지 않음 | — | **I2** | sbh3821 | ✅ |
...
| I2 | 캠페인 전체 통합 | 인트로→게시판→탐험→정산→월드턴→다음 공고/엔딩이 같은 시드로 재현되고 전체 검증 통과 | — | **B1 Q1** | sbh3821 | ✅ |
...
- [대표 화면](../diagram/screens.md) — 구현된 화면을 그대로 캡처한 일곱 장
