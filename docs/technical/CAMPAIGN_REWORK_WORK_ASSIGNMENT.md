...
**이 표가 현재 유일한 작업 기준이다.** 아래 37개 항목을 모두 완료하면 인트로부터 캠페인 엔딩까지 동작한다. 이전 개편의 배정표는 다시 갱신하지 않으며 이 문서를 읽는 데 필요하지도 않다. 그 표에도 `F1`~`Q2`라는 글자가 있지만 완전히 다른 작업이다.
...
`F2-1`·`F2-2`가 끝나 테마 콘텐츠도 마무리됐다. 지금 시작 가능한 것은 콘텐츠 둘(`F3`·`F5`), 규칙 둘(`C3`·`E1`), 화면 셸(`U1`), 그리고 대표 화면 이미지 작업(`D8`)이다. 여섯 개가 서로 파일이 겹치지 않는다. 임계 경로는 `E1`이 아니라 이제 `E2`로 넘어간다 — `E2`가 `F3`을 기다리기 때문이다.
...
| D7 | 상태·시퀀스 다이어그램 이미지 재생성 | 캠페인·탐험 상태 및 시퀀스 4장의 SVG·PNG가 최신 md에서 재생성되고 PR #58에 반영됨 | — | **Q1** | LatteBun | ✅ |
| D8 | 대표 화면 이미지 재작업 | 인트로를 포함한 대표 화면 6장의 SVG·PNG와 전체 모음이 최신 화면 설계에 맞게 재작업되고, 텍스트·밀도·순차 턴제 보스전 정보가 확인됨 | — | **Q1** | LatteBun | ⬜ |
