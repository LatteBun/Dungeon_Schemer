...
문서 계층과 기존 도메인 계약이 끝났다. 새 조언·사건 계약은 `D9` 문서 개정과 `F1-2` 타입 개정에서 시작한다. `F1`이 새 타입을 놓으면서 **`C3`·`E1`·`U1`의 선행이 모두 풀렸다.**

`F2-1`·`F2-2`가 끝나 테마 콘텐츠도 마무리됐다. 지금 시작 가능한 것은 조언·사건 문서(`D9`), 콘텐츠(`F5`), 규칙 둘(`C3`·`E1`), 화면 셸(`U1`), 그리고 대표 화면 이미지 작업(`D8`)이다. 여섯 개가 서로 파일이 겹치지 않는다. `F3`은 더 이상 시작 가능한 작업이 아니다.
