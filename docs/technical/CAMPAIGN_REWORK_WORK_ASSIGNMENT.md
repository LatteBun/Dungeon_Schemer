...
| C7 | 캠페인 전이·백테스트 | 정산 ID 중복 거부를 포함한 게시판부터 엔딩까지 순수 전이. 즉시 불신에서는 신뢰 변화 묶음 뒤 최신 활성 파티를 source of truth로 받아 계약 파티와의 서로 다른 3명·ID 일치를 검증하고 `phase: ended`를 원자 기록한다. 의도 3종×추론 정확도 2종×10,000시드 보고서, 생성 오류/시작 진행 불가 0건 | — | **B1 I1** |  | ⬜ |
