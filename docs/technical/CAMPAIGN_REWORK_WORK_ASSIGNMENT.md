...
| C6 | 엔딩·신뢰 0 누적 | 엔딩 5종 우선순위, 응급 후보 평가 뒤 인력 소진, 신뢰 0 누적 2~4명 수용·적발 보정, 5명 즉시 누적 고발 | — | **C7 U6** |  | ⬜ |
| C7 | 캠페인 전이·백테스트 | 정산 ID 중복 거부를 포함한 게시판부터 엔딩까지 순수 전이, 의도 3종×추론 정확도 2종×10,000시드 보고서, 생성 오류/시작 진행 불가 0건 | C5 C6 | **B1 I1** |  | ⬜ |
| C8 | 캠페인 통계 | SettlementResult를 재계산하지 않고 조언·반응, 생환·전멸, 전환점, 원정 연대기와 정산 원인 사슬을 캠페인 상태에 누적 | — | **U6** |  | ⬜ |
...
| U3 | 게시판·계약·승급 화면 | 공고 최대 5장, 진입 불가 표시/사유, 상세 던전 정보·출전 3인·계약 보상과 버튼, 공개 환경 특성 없음. 상단 등급 버튼의 승급 가능 강조, 명성/골드 선택, 결과 확인 뒤 갱신된 공고 표시 | C5 | **I2** | SangHwan Yoo | ✅ |
...
| U6 | 정산·엔딩 화면 | 정산 원인 순서, 위험도·보상 변화와 승급 가능 여부, 엔딩 5종 판정 원인·누적 통계 표시. 승급 버튼·선택·결과는 제공하지 않음 | C6 C8 | **I2** | sbh3821 | 🟡 |
...
`U6`는 화면과 ViewModel 경계까지 만들었고 `/u6-test`에서 정산 3종·엔딩 5종을 확인할 수 있다. 선행 `C5`·`C6`·`C8`이 아직 없어 실제 `CampaignState`를 읽지 않고 결정적 fixture로 그린다. C4의 `SettlementResult` 어댑터는 이미 준비되어 있으며, 남은 규칙이 들어오면 `components/game/u6-preview-data.ts` 자리에 View 타입을 만드는 함수가 들어오고 화면 코드는 그대로다. 그 연결이 `I2`의 몫이므로 상태는 `🟡`로 둔다.
