...
| U5-2 | 자동 전투 장면 연출 | 던전 진행 화면 좌측 상단 장면 슬롯에 정적 PNG 캐릭터·몬스터를 배치하고 Framer Motion으로 `Idle → Attack Lunge → Hit Shake → Damage Number → HP Bar 감소`를 순차 재생한다. 일반 몬스터 장면은 E3가 확정한 사건·전투 결과를, 보스전은 E4 턴 기록을 소비하며 UI에서 피해·RNG·신뢰를 재계산하지 않는다 | U5 E3 E4 | **I2** | sbh3821 | 🟡 |
...
| I2 | 캠페인 전체 통합 | 인트로→게시판→탐험→정산→월드턴→다음 공고/엔딩이 같은 시드로 재현되고 전체 검증 통과 | U5 U5-2 U6 I1 | **B1 Q1** |  | ⬜ |
...
일반 몬스터 전투는 `E3`의 `resolveMonsterEventBattle`이 계산한 결과를 **실제로
소비한다.** 보스전은 `E4`가 없어 `resolveBossBattle`을 호출하지 않는 시각
fixture이며, 화면에도 그렇게 표시한다. 공식 콘텐츠 27종의 전투 이미지 매핑은
끝났다. `E4`가 들어오면 fixture 자리만 바뀌고 화면 코드는 그대로다. 그 연결이
`I2`의 몫이므로 상태는 `🟡`로 둔다.
