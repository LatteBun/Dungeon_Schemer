...
E1 이후의 탐험 규칙은 후속 설계로 구체화되었다. E2는 C1이 확정한 활성 생태 규칙을 소비해 현재 위험도별 공개 규칙과 조언 판정, 보스 정보 Depth 계획을 완료했고, E3가 그 결과를 받아 실제 사건 물질화와 연계를 수행한다.
...
E2의 생태 공개·조언 판정·보스 정보 Depth 계획이 완료되어 E3가 바로 그 결과를 소비해 실제 사건·단서·연계를 만들 수 있다.
...
| E2 | 생태·조언 판정 | C1 `activeRuleIds` 3개를 재추첨하지 않고 현재 위험도별 3/2/1개를 결정적 부분집합으로 공개하며, 조건부 규칙은 사건별 성립 선언이 있을 때만 후보가 된다. 조언 3개를 결정적으로 섞고 살아 있는 파티원별 기본 확률+신뢰 구간+성격+C6 보정으로 수용·의심·적발을 독립 판정한다. 한 명 이상 수용 시 `executed`, 전원 미수용 시 기본 결과를 선택하며 즉시 신뢰와 보스 지연 신뢰 시점을 구분한다. 보스 정보 Depth를 ★1~2 1개, ★3~5 2개 예약하고 현재 던전 보스가 아닌 정보를 거부한다 | — | **E3 E4 U5** | LatteBun | ✅ |
| E3 | 사건 배치·단서·연계 | E2 `BossInfoDepthPlan`을 입력으로 받아 예약 Depth와 강한 연계 Depth가 충돌하지 않게 계획한다. 실제 방문 사건 ID가 한 원정에서 중복되지 않고 `monster`·`rest`·`merchant`·`special` 경로 보장을 만족하며, 방문 단서가 누적되어 약한 연계 슬롯 교체와 강한 연계 사건 물질화에 쓰인다. E2 `executed`를 소비해 사건 효과/기본 결과를 한 번 적용하고 merchant 결제·즉시/다음 전투 효과·pending 폐기를 처리한다 | — | **E4 U5 U5-2** |  | ⬜ |
| E4 | 턴 단위 보스전 | 자동 전투가 턴별 행동·피해 기록을 남기고 수용한 보스 조언 modifier와 누적 상하한을 적용하며, 전투 뒤 E2 pending help/harm·의심 기록을 결과 기반 신뢰 행동으로 검증 | E3 | **C4 U5-2** |  | ⬜ |
