...
테마 콘텐츠(`F2-1`·`F2-2`)는 끝났으니 이제 조언 풀(`F3`)과 그 위의 추론 판정(`E2`)이 병목이다. `E2`는 거미굴 fixture로 먼저 시작할 수 있지만, 완료하려면 `F3`의 조언 유형 조합이 필요하다.
...
| D3 | 던전·테마·정보 문서 | 초기 위험도별 6~8 지점표, 재도전 규칙, 일반 지점의 조언 기회와 보스 조언 보장표, 테마 3종의 생태 규칙 계약이 적히고, 선택 전 조언 유형·발각 위험을 보여준다는 조항이 삭제됨 | — | **D6 F2-1 F3 F5** | LatteBun | ✅ |
...
| E2 | 생태 규칙 추론 판정 | 던전마다 활성 규칙 3개를 뽑고 위험도별로 3/2/1개를 공개하며, 조언의 정합·모순이 활성 규칙으로 판정되고, 살아 있는 파티원별 수용·의심·적발을 독립 판정해 신뢰를 갱신하며, 모든 일반 지점의 조언 1회와 보스 관련 보장 1~2회를 모든 경로가 충족 | F3 E1 | **E4 U5** |  | ⬜ |
| E3 | 사건·아이템 | 몬스터·휴식·상인·특수 네 분류가 모든 경로에 한 번 이상 나오고 한 던전 안에서 사건 콘텐츠가 중복되지 않으며, 조언 결과와 아이템 효과가 HP·신뢰·골드를 바꾸는 테스트 통과 | E1 F5 | **E4 U5** |  | ⬜ |
...
| U5 | 던전 진행 화면 | 좌측이 상 40%·하 60%로 나뉘고, 상황 묘사와 조언 3개가 같은 디자인으로 유형·발각 위험·예상 신뢰 변화를 감추며, 선택 뒤 반응·사건·수치 변화가 인과 순서로 나타남 | E2 E3 | **I2** |  | ⬜ |
...
| 조언 주제 6종 × 유형 3종 × 2개 구조 | 모든 일반 지점이 조언 기회가 되어도 콘텐츠 수량 계약이 유지된다. `F3`은 태그를 더한다 |
