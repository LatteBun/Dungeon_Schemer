...
`F2-1`·`F2-2`가 끝나 테마 콘텐츠도 마무리됐다. 지금 담당자 없이 시작할 수 있는 것은 콘텐츠 둘(`F3`·`F5`), 규칙 둘(`C3`·`E1`), 화면 셸(`U1`), 그리고 `D7`이다. 여섯 개가 서로 파일이 겹치지 않는다. 임계 경로는 `E1`이 아니라 이제 `E2`로 넘어간다 — `E2`가 `F3`을 기다리기 때문이다.

### 임계 경로

```text
D1 → D3 → F3 → E2 → E4 → C4 → C6 → C7 → I1 → I2 → B1 → Q1 → Q2
```

테마 콘텐츠(`F2-1`·`F2-2`)는 끝났으니 이제 카드 풀(`F3`)과 그 위의 추론 판정(`E2`)이 병목이다. `E2`는 거미굴 fixture로 먼저 시작할 수 있지만, 완료하려면 `F3`의 카드 진위 조합이 필요하다.
...
| C1 | 캠페인 초기화 | 5직업×6명·5성격×6명의 30명 풀, 테마 3종×5개와 초기 위험도 3/4/4/3/1의 던전 15개, 시작 명성 30·골드 10이 시드로 재현되는 테스트 통과 | F4 | **C2 C7** |  | ⬜ |
...
| E2 | 생태 규칙 추론 판정 | 던전마다 활성 규칙 3개를 뽑고 위험도별로 3/2/1개를 공개하며, 카드의 정합·모순이 활성 규칙으로 판정되고, 살아 있는 파티원별 수용·의심·적발을 독립 판정해 신뢰를 갱신하며, 위험도별 정보 기회 2~5회와 보스 관련 보장 1~2회를 모든 경로가 충족 | F3 E1 | **E4 U5** |  | ⬜ |
| E3 | 사건·아이템 | 몬스터·휴식·상인·특수 네 분류가 모든 경로에 한 번 이상 나오고 한 던전 안에서 사건 콘텐츠가 중복되지 않으며, 사건 행동과 아이템 효과가 HP·신뢰·골드를 바꾸는 테스트 통과 | E1 F5 | **E4 U5** |  | ⬜ |
| E4 | 턴 단위 보스전 | 자동 전투가 턴별 행동·피해 기록을 남기고, 수용한 보스 정보의 진위별 보정과 누적 상하한이 적용되며, 전투 뒤 거짓·의심 기록이 검증되어 신뢰가 갱신되는 테스트 통과 | E2 E3 | **C4** |  | ⬜ |
