...
`B1`은 두 단계다. 먼저 **B1-A**가 현행 Store와 규칙을 그대로 돌리는 headless
백테스트로 생존·균형·선별적 배신 3전략과 추론 정확도 0.4·0.7을 진단한다.
calibration 결과로 합격 수치를 사용자와 합의한 뒤 별도 holdout 시드로 판정한다.
현행 규칙이 기준을 통과하면 B1을 끝내고, 실패하면 보고서 승인 뒤에만 **B1-B**
재설계를 연다. B1-B는 필요하면 프로토타입 고정 범위까지 다시 검토할 수 있지만,
B1-A가 원인을 측정하기 전에는 게임 규칙을 바꾸지 않는다.
...
| B1 | 밸런스 재측정 | **B1-A:** 실제 Store를 쓰는 결정적 headless 백테스트가 공개 정보만 보는 생존·균형·선별적 배신 3전략 × 추론 정확도 0.4·0.7을 calibration·holdout 시드로 비교하고, 생성 오류·거부·정체 0건, S 도달률 100% 미만, 배신 정상 완주 사례, 통계적·실질적 정확도 차이를 판정한 재생성 보고서를 낸다. calibration 뒤 수치 기준은 사용자 승인으로 고정한다. 실패하면 보고서 승인 뒤 **B1-B** 별도 설계로 규칙·고정 범위를 조정하고 새 holdout으로 재측정한다 | — | **Q1** |  | ⬜ |
