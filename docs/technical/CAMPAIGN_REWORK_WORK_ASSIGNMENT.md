...
| C2 | 게시판·임시 파티 편성 | 진입 가능한 미클리어 던전을 위험도 높은 순으로 최대 5개 노출하고, 부족하면 가장 가까운 진입 불가 던전을 사유와 함께 채우며, 공고마다 서로 다른 직업 정확히 3명의 완전한 임시 파티를 제공하고, 같은 게시판 안에서 캐릭터를 중복 배정하지 않으며 시드·월드턴 재현 테스트를 통과 | — | **C4 U3** | SangHwan Yoo | ✅ |
...
| U3 | 게시판·계약 화면 | 공고 최대 5장이 게시판 위에 배치되고 진입 불가가 붉은 테두리와 사유로 구분되며, 상세에서 던전 정보·출전 3인 카드·계약 보상과 계약 버튼을 보여줌 | — | **I2** | SangHwan Yoo | ✅ |
