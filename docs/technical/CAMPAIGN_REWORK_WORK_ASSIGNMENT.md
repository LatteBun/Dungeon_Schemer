...
| I2 | 캠페인 전체 통합 | 인트로→게시판→탐험→정산→월드턴→다음 공고/엔딩이 같은 시드로 재현되고 전체 검증 통과 | — | **B1 Q1** |  | ⬜ |
...
`I2`는 아직 `🟡`이다. 인트로→게시판→계약→지도→진행→조언→지도의 왕복이 `/campaign`
한 페이지에서 돌고, 같은 시드가 같은 캠페인을 낸다. 남은 것은 원정 뒤쪽이다 —
보스전 재생(`U5-2`) 붙이기, 정산·월드턴 잇기, 엔딩 View 어댑터다.
