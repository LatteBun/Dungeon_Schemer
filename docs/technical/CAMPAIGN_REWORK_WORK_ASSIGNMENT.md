...
**이 표가 현재 유일한 작업 기준이다.** 아래 42개 항목을 모두 완료하면 인트로부터 캠페인 엔딩까지 동작한다. 이전 개편 배정표와 오래된 회의록은 역사 자료이며 현재 작업 상태를 덮어쓰지 않는다.
...
- 보스전 턴 기록의 최종 시각 표현. 규칙이 턴 기록을 남기는 데까지가 이번 범위다
- 저장·복원, Supabase, 로그인
...
| 화면 | `U1`~`U6` | 셸과 주요 화면 | 규칙 결과를 설명하는 사용자 흐름 |
...
| E3 | 사건 배치·단서·연계 | E2 `BossInfoDepthPlan`을 입력으로 받아 예약 Depth와 강한 연계 Depth가 충돌하지 않게 계획한다. 실제 방문 사건 ID가 한 원정에서 중복되지 않고 `monster`·`rest`·`merchant`·`special` 경로 보장을 만족하며, 방문 단서가 누적되어 약한 연계 슬롯 교체와 강한 연계 사건 물질화에 쓰인다. E2 `executed`를 소비해 사건 효과/기본 결과를 한 번 적용하고 merchant 결제·즉시/다음 전투 효과·pending 폐기를 처리한다 | E1 E2 F3-1 F3-2 F3-3 | **E4 U5** |  | ⬜ |
| E4 | 턴 단위 보스전 | 자동 전투가 턴별 행동·피해 기록을 남기고 수용한 보스 조언 modifier와 누적 상하한을 적용하며, 전투 뒤 E2 pending help/harm·의심 기록을 결과 기반 신뢰 행동으로 검증 | E2 E3 | **C4** |  | ⬜ |
...
| U5 | 던전 진행 화면 | 좌측 상단 장면 슬롯과 하단 플레이어 콘솔 `[행동 / 조언] [진행 기록]`을 두고 진행 기록은 `[전체][단서][전투][생태]`를 제공한다. `생태`는 확인된 생태와 관찰 단서를 구분하고 숨은 규칙을 자동 정답 처리하지 않는다. 조언 3개는 같은 디자인으로 내부 유형·정합 관계·확률·예상 신뢰를 숨기며 선택 뒤 반응→사건 결과→수치/신뢰 변화를 인과 순서로 표시 | E2 E3 | **I2** |  | ⬜ |
...
| I2 | 캠페인 전체 통합 | 인트로→게시판→탐험→정산→월드턴→다음 공고/엔딩이 같은 시드로 재현되고 전체 검증 통과 | U4 U5 U6 I1 | **B1 Q1** |  | ⬜ |
