...
| E4 | 턴 단위 보스전 | `boss-battle-adapter`가 보스와 보스 정보·merchant pending을 공통 BattleEngine 입력으로 변환하고, 턴별 action record와 전투 뒤 E2 pending help/harm·의심 기록 검증을 연결한다 | E3 | **C4 U5-2** |  | 🟡 |
...
| U5 | 던전 진행 화면 | 좌측 상단 장면 슬롯과 하단 플레이어 콘솔 `[행동 / 조언] [진행 기록]`을 두고 진행 기록은 `[전체][단서][전투][생태]`를 제공한다. `생태`는 확인된 생태와 관찰 단서를 구분하고 숨은 규칙을 자동 정답 처리하지 않는다. 조언 3개는 같은 디자인으로 내부 유형·정합 관계·확률·예상 신뢰를 숨기며 선택 뒤 반응→사건 결과→수치/신뢰 변화를 인과 순서로 표시 | E3 | **U5-2 I2** | sbh3821 | 🟡 |
...
`U5`도 화면과 ViewModel 경계까지 만들었고 `/u5-test`에서 아홉 상태를 확인할 수 있다. 선행 `E3`가 진행 중이지만 `E2` 조언 판정이 완료돼 있어, 조언 제시·반응 판정·생태 공개는 **실제 규칙이 한다.** `components/game/u5-preview-data.ts`가 지어내는 것은 어떤 사건이 나왔는가 하나뿐이고, `E3`가 들어오면 그 자리만 바뀐다. 그 연결이 `I2`의 몫이므로 상태는 `🟡`로 둔다.
