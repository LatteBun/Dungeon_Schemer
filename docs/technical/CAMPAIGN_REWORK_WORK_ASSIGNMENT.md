...
| D7 | 다이어그램 이미지 재생성 | 상태·시퀀스 4장과 대표 화면 6장의 svg·png가 갱신된 md에서 재생성되고, 이미지 재생성 대기 문구가 문서에서 제거됨 | — | **Q1** | LatteBun | 🟡 |
...
| C7 | 캠페인 전이·백테스트 | 게시판부터 엔딩까지 순수 전이 함수로 진행되고, 의도 3종 × 추론 정확도 2종의 6전략으로 10,000시드 보고서가 생성되며, 생성 오류와 시작 즉시 진행 불가 시드가 0건 | C1 C5 C6 | **B1 I1** |  | ⬜ |
...
- 대표 화면 6장(인트로 포함)의 SVG·PNG는 품질 재검토가 필요해 이번 작업에서 제외했다. 해당 화면과 전체 모음은 별도 재작업 뒤 D7을 완료 처리한다.
