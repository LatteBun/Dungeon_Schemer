...
지금 **38개가 끝났고 5개가 남았다.** 문서·기반·캠페인 규칙·탐험 규칙·화면이 모두 완료됐고, 남은 것은 마감 다섯이다.
...
화면 일곱은 모두 끝났다. 지금 **선행 없이 시작할 수 있는 것은 `I1` 하나**이고,
나머지는 그 뒤에 한 줄로 이어진다. `I1`이 유일한 병목이다.
...
| I1 | 상태 머신·스토어 | C7 `transitionCampaign`을 호출하고 persistent `CampaignState`와 분리된 `CampaignTransitionContext`를 세션 Store에 보관하며 인트로·게시판·계약·지도·조언·보스전·정산·월드턴·엔딩 화면에 결과를 공급 | — | **I2** |  | ⬜ |
| I2 | 캠페인 전체 통합 | 인트로→게시판→탐험→정산→월드턴→다음 공고/엔딩이 같은 시드로 재현되고 전체 검증 통과 | I1 | **B1 Q1** |  | ⬜ |
