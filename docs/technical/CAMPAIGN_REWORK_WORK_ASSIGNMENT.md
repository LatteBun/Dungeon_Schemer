...
| U1 | 3:1 게임 셸 | 상단 상태 바와 좌 75%·우 25% 본문을 가진 공통 셸 위에서 모든 화면이 콘텐츠만 교체하고, 1280×720 기준과 1024×640 최소에서 가로 스크롤이 생기지 않음 | — | **U2 U3 U4 U5 U6** | SangHwan Yoo | ✅ |
...
| U5 | 던전 진행 화면 | 좌측이 상 40%·하 60%로 나뉘고, 카드 3장이 같은 디자인으로 유형·발각 위험·예상 신뢰 변화를 감추며, 선택 뒤 반응·사건·수치 변화가 인과 순서로 나타남 | E2 E3 | **I2** |  | ⬜ |
...
- 2026-08-20: `GameShell`·`TopStatusBar`·`U1Preview`와 `/u1-test` 프리뷰 하네스를 구현했다. 인트로를 포함한 다섯 화면이 같은 셸의 콘텐츠 슬롯만 교체한다.
- 본문은 모든 화면과 상태에서 `minmax(0, 3fr) minmax(0, 1fr)`을 유지한다. 인트로도 우측 25% 레일을 비우지 않고 구조적으로 유지한다.
- 실제 Chromium 브라우저에서 1280×720과 1024×640을 확인했다. 두 viewport 모두 계산된 grid track 비율은 `3:1`, `documentScrollWidth === innerWidth`, console/page error 0건이었다.
- 다섯 화면 클릭 전환, `aria-pressed`, Tab·Enter 키보드 전환과 Next 오류 overlay 부재를 확인했다. 스크린샷은 두 viewport에서 생성했다.
- `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`를 통과했다. 이후 `U2`~`U6`는 `GameShell`·`TopStatusBar`와 두 슬롯 계약을 재사용한다.
