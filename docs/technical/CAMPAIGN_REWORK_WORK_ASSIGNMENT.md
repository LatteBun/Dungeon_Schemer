...
`U5`의 사건 물질화 연결은 막혀 있다. `E3`를 **실제 `CampaignDungeon` 프로필로**
부르면 활성 규칙 3개와 출현 잡몹 1~2종만 넘어가는데, 그 조합에서 `monster`
사건이 하나도 후보에 남지 않는다. 시드 3개 × 던전 15개 × attempt 2회 = 90개
조합으로 재 보면 `prepareExpeditionEvents`가 33번 던지고(`요구된 strong link
수를 만족하는 전역 후보 조합이 없다`), 준비에 성공한 57개 중 23개가 `monster`
노드를 계획해 놓고 하나도 물질화하지 못한다(`방문 노드의 사용 가능한 사건이
없다`). `E3`의 자체 테스트는 테마 **전체** 규칙·몹을 넘기므로 이 자리를 지나간다.
`C1`은 문서대로다 — 생태 패키지는 「활성 규칙 3개·출현 잡몹 1종 이상」이다.
`E3`의 필터가 좁은 것인지 `F3` 콘텐츠가 모자란 것인지는 그 담당이 판단할 일이다.
