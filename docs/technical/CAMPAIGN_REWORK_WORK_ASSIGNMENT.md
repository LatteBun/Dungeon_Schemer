...
| F3-2 | 조언 콘텐츠 · 거미굴 | 거미굴 생태 규칙을 참조하는 전용 사건 16개가 있고, 규칙마다 도움 2개·방해 2개 이상이며 중립은 테마 전체에 2개 이상이고, 단서를 남기는 사건과 강한 연계 사건이 각각 1개 이상 존재하며 검증 통과. 검증기가 테마의 규칙 목록을 받아 어느 조언도 참조하지 않은 규칙까지 검사하고, 테마에 없는 규칙을 가리키는 조언을 거부함 | — | **E2 E3 F3-5** |  | ⬜ |
...
| E3 | 사건 배치·단서·연계 | 몬스터·휴식·상인·특수 네 분류가 모든 경로에 한 번 이상 나오고 한 던전 안에서 사건이 중복되지 않으며, 방문한 사건의 단서가 누적되어 약한 연계가 슬롯을 교체하고, 위험도별 강한 연계 쌍이 전 경로 공통 지점에 배치되는 테스트 통과 | E1 F3-2 F3-3 F5 | **E4 U5** |  | ⬜ |
