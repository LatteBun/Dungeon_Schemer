...
문서·기반·캠페인 규칙·탐험 규칙이 모두 완료됐다. 화면도 `U6`만 남았다.
남은 것은 한 줄이다.
...
화면 일곱과 스토어가 끝났다. 지금 **선행 없이 시작할 수 있는 것은 `I2` 하나**이고,
나머지는 그 뒤에 한 줄로 이어진다.
...
한 바퀴는 닫혔다. 인트로→게시판→계약→지도→진행→조언→보스전→정산→월드턴→
게시판이 `/campaign` 한 페이지에서 돌고, 같은 시드가 같은 캠페인을 낸다. 승급을
밟아 등급 S까지 올라 엔딩에 이르는 한 판도 검사로 돌린다.

**막는 것은 `E3`다.** 지도가 규칙이 거부할 이동을 내놓는다 — 강한 연계의 후속
지점은 선행 단서를 들고 있어야 물질화되는데, 배치는 「선행에서 도달 가능」만
요구한다. 갈림길에서 다른 갈래로 가면 선행을 밟지 않고 후속에 닿고, 그 지점은
지도에서 고를 수 있게 보인다. 실제 플레이에서 막다른 길이다.

**40시드 중 23이 도중에 막힌다.** 완료 기준의 「같은 시드로 재현」은 지금도
참이지만 「인트로→…→엔딩」이 시드의 절반에서 성립하지 않는다. 그래서 `✅`로
두지 않았다. `#129`가 그 몫이고, 재현 측정은
`lib/store/campaign-full-run.test.ts`의 `알려진 결함`에 검사로 남겼다 — 고쳐지면
그 검사가 빨개져 알려 준다.
