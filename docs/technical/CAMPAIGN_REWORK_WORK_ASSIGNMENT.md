...
**이 표가 현재 유일한 작업 기준이다.** 아래 42개 항목을 모두 완료하면 인트로부터 캠페인 엔딩까지 동작한다. 이전 개편의 배정표는 다시 갱신하지 않으며 이 문서를 읽는 데 필요하지도 않다. 그 표에도 `F1`~`Q2`라는 글자가 있지만 완전히 다른 작업이다.
...
| 기반 | `F1` `F1-2` `F2-1` `F2-2` `F3-1`~`F3-5` `F4` | 도메인 계약과 조언·테마·캐릭터 콘텐츠 데이터 | 타입과 데이터 |
...
지금 시작 가능한 것은 대표 화면 이미지 작업(`D8`), **조언 콘텐츠 거미굴(`F3-2`)**, 사막·묘지(`F3-3`), 공용 사건 확장(`F3-4`), 게시판·편성(`C2`), 위험도별 지도(`E1`)이다. 서로 파일이 겹치지 않는다.
...
| D3 | 던전·테마·정보 문서 | 초기 위험도별 6~8 지점표, 재도전 규칙, 일반 지점의 조언 기회와 보스 조언 보장표, 테마 3종의 생태 규칙 계약이 적히고, 선택 전 조언 유형·발각 위험을 보여준다는 조항이 삭제됨 | — | **D6 F2-1** | LatteBun | ✅ |
...
| F3-1 | 조언 콘텐츠 계약·검증기와 공용 사건 | 사건 검증기가 조언 3개·도움방해중립 각 1개·기본 결과·중복·빈 문구를 검사하고 위반 시 생성 오류를 반환하며, 생태 규칙을 참조하지 않는 공용 사건이 `rest`·`merchant`·`special` 각 5개 이상이며 검증을 통과하고, 강화판의 대체 조언이 교체할 슬롯과 다른 결과 유형이면 검증기가 거부함 | — | **F3-2 F3-3 E3** | LatteBun | ✅ |
| F3-2 | 조언 콘텐츠 · 거미굴 | 거미굴 전용 사건 20개(일반 `monster` 12개 + 보스 정보 `special` 8개)가 있고, 생태 규칙마다 도움 2개·방해 2개 이상이며 보스 4종마다 전용 정보 2개가 있다. 약한 연계 1세트와 강한 연계 2세트가 존재하고 검증을 통과한다. 검증기는 테마의 규칙 목록을 받아 어느 조언도 참조하지 않은 규칙까지 검사하고, 테마 밖 생태 규칙·다른 보스의 특징 참조를 거부한다. 보스 정보 사건은 대상 보스·전용 source·세 modifier를 모두 가져야 한다 | — | **E2 E3 F3-5** | LatteBun | ✅ |
| F3-3 | 조언 콘텐츠 · 사막·묘지 | 사막·묘지 전용 사건이 각각 20개(일반 monster 12 + 보스 정보 special 8)이며, 각 생태 규칙이 도움 2개·방해 2개 이상을 공급하고 각 보스가 전용 정보 2개를 가진다. 테마별 약한 연계 1세트와 강한 연계 2세트가 존재하며 기존 테마·보스 source 검증을 통과한다. | — | **E2 E3 F3-5** | LatteBun | ✅ |
| F3-4 | 공용 사건 확장 | 공용 사건이 `rest`·`merchant`·`special` 각 30개로 늘고 검증기를 그대로 통과하며, 사건 ID·문구 중복이 없음 | — | — | LatteBun | ✅ |
...
| C2 | 게시판·임시 파티 편성 | 진입 가능한 미클리어 던전을 위험도 높은 순으로 최대 5개 노출하고, 부족하면 가장 가까운 진입 불가 던전을 사유와 함께 채우며, 공고마다 생태 근거가 있는 공개 환경 특성과 서로 다른 직업 정확히 3명의 완전한 임시 파티를 제공하고, 같은 게시판 안에서 캐릭터를 중복 배정하지 않으며 시드·월드턴 재현 테스트를 통과 | — | **C4 U3** | SangHwan Yoo | ✅ |
| C3 | 월드턴 | HP 50% 미만 강제 휴식, 나머지의 휴식·백그라운드 절반 배정, 최대 HP 비례 회복, HP 하한 1, 20% 미만 중상과 다음 턴 출전 불가가 동작하고 백그라운드 사망 0건 | — | **C7** | sanghwan.yoo | ✅ |
| C4 | 정산·위험도 상승 | 생존 3/2/1명의 100·60·30% 보상, 전멸 시 유품 회수와 상승 전 위험도 기준 명성 손실·하한 0, 던전 위험도 +1과 ★5 상한, 상승한 위험도의 보상표가 다음 공고에 반영되는 테스트 통과 | E4 | **C5 C6 C8 U6** |  | ⬜ |
...
| E1 | 위험도별 지도 생성 | 초기 위험도로 정해진 6~8 일반 Depth가 실패 뒤에도 고정되고, 각 Depth에 1~5개 노드가 있으며 첫·마지막 폭은 최대 2이고, 한 노드의 incoming/outgoing이 각각 최대 2이며 모든 노드가 Entry에서 도달 가능하고 Boss로 이어진다. 모든 경로 길이는 같고, 16개 위험도 전용 폭 템플릿과 결정적 재도전 순환을 사용하며, `GeneratedMap.layers`와 `INVALID_GENERATION` 검증을 제공한다 | — | **E2 E3 U4** | lattebun | ✅ |
| E2 | 조언 판정 | 던전마다 활성 규칙 3개를 뽑고 위험도별로 3/2/1개를 공개하며, 조언의 정합·모순이 활성 규칙으로 판정되고, 살아 있는 파티원별 수용·의심·적발을 독립 판정해 한 명이라도 수용하면 실행하며, 아무도 수용하지 않으면 기본 결과가 나오고 보스 정보는 Depth 슬롯에서 실제 방문 시 물질화되어 모든 경로가 1~2회를 충족 | — | **E4 U5** |  | ⬜ |
| E3 | 사건 배치·단서·연계 | 몬스터·휴식·상인·특수 네 분류가 모든 경로에 한 번 이상 나오고 실제 방문 사건 ID가 한 원정에서 중복되지 않으며, 방문한 사건의 단서가 누적되어 약한 연계가 슬롯을 교체하고, 위험도별 강한 연계 쌍이 서로 다른 Depth 슬롯에 배치되어 실제 방문 시 물질화된다. merchant는 비용 확인, 실제 실행 시 결제, 즉시/다음 전투 효과 적용, 던전 종료·전멸·정산 진입 시 pending 폐기를 모두 통과한다 | — | **E4 U5** |  | ⬜ |
...
| U3 | 게시판·계약 화면 | 공고 최대 5장이 게시판 위에 배치되고 진입 불가가 붉은 테두리와 사유로 구분되며, 상세에서 던전 정보·공개 환경 특성·출전 3인 카드·계약 보상과 계약 버튼을 보여줌 | — | **I2** | SangHwan Yoo | ✅ |
| U4 | 던전 지도 화면 | 범례 영역이 없고 지점이 아이콘으로 표현되며, 현재 위치·방문 완료·선택 가능·비활성·보스방이 색과 형태로 함께 구분되고 키보드로 다음 지점을 선택할 수 있음 | — | **I2** |  | ⬜ |
...
| I2 | 캠페인 전체 통합 | 인트로→게시판→탐험→정산→월드턴→다음 공고 또는 엔딩이 스토어 위에서 이어지고 같은 시드로 재현되며 전체 검증 통과 | U4 U5 U6 I1 | **B1 Q1** |  | ⬜ |
...
- 2026-08-20: `createBoardOffers`가 접근 가능한 미클리어 던전을 위험도 내림차순으로 최대 5개 게시하고, 부족한 슬롯에는 가장 가까운 잠금 던전을 `rankTooLow` 사유와 함께 채운다.
- 공고 환경 특성은 테마별 후보 3개 중 생태 패키지의 출현 잡몹 특성이 근거가 되는 하나로 고정하며 콘텐츠 검증이 이를 보장한다.
- 각 공고는 서로 다른 직업 정확히 3명의 임시 파티를 가지며, 같은 게시판 안에서는 캐릭터를 중복 배정하지 않는다. 완전한 파티를 더 만들 수 없으면 공고 수를 줄인다.
- 같은 시드·월드턴은 같은 게시판을 재현하고, 월드턴이 달라지면 공고와 파티를 새로 구성한다.
...
- 2026-08-21: PR 70에서 `/u3-test`와 `U3BoardScreen`·`U3Preview`·U3 화면 모델을 구현해 C2 공고의 최대 5장, 진입 가능 여부와 잠금 사유, 던전·공개 환경 특성·출전 3인·계약 보상을 화면에 연결했다.
- 선택한 공고의 상세 패널과 계약 버튼, 위험도별 생존 보상, C2 임시 파티의 캐릭터 정보를 표시하며, 화면 모델·컴포넌트·에셋·반응형 레이아웃 테스트를 추가했다.
- `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`와 1280×720·1024×640·1920×1080 브라우저 검증을 통과했다. C2·C3·정산·전체 전이는 후속 규칙·통합 작업의 책임이며 U3 프리뷰는 이를 선행 구현하지 않는다.
...
| 테마별 조언 콘텐츠 | 거미굴·사막·묘지 전용 대표 콘텐츠를 테마당 30개씩 제공하고 활성 규칙별 유형 조합과 단서·강한 연계를 채운다 | `F3-2` `F3-3` `F3-5` |
