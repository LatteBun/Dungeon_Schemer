...
문서 계층과 기존 도메인 계약이 끝났다. 새 조언·사건 계약은 `D9` 문서 개정과 `F1-2` 타입 개정에서 시작한다. `C1`이 끝나 `C2`의 선행도 풀렸다.

지금 시작 가능한 것은 대표 화면 이미지 작업(`D8`), 조언·사건 문서(`D9`), 아이템 콘텐츠(`F5`), 게시판·편성(`C2`), 위험도별 지도(`E1`), 인트로 화면(`U2`)이다. 여섯 개가 서로 파일이 겹치지 않는다. `F3-1`~`F3-3`은 `F1-2`를 기다리므로 아직 시작할 수 없다.
