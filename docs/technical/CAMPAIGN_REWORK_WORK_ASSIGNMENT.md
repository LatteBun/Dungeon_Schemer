...
| M1 | 메인 메뉴·업적 기록 | 엔딩 결과를 실행 ID당 한 번만 순수 규칙으로 누적하고, V1 브라우저 프로필 저장과 `/` 메인 메뉴·`/achievements` 4×2 기록 UI를 제공한다. 캠페인 Store 영속화와 규칙 보상은 포함하지 않음 | — | — | sbh3821 | ✅ |
| B1 | 밸런스 재측정 | 배신 전략 성립, S 도달률 100% 미만, 추론 정확도 0.4와 0.7 결과 차이가 유의하며 상수 변경과 백테스트 보고서 동봉 | — | **Q1** |  | ⬜ |
