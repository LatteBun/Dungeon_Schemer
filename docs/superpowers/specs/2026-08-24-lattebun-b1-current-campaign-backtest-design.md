...
- 각 전략의 같은 시드 0.7−0.4 정상 완주 paired 차이가 `0.05` 이상이고, paired
  95% 신뢰구간 하한이 0보다 커 통계적 차이와 플레이상 실질 차이를 함께 가짐.
  `0.05`는 B1-B 승인 완주율 구간에서 요구되는 정확도 간격 중 가장 작은 값
  (`selective-betrayal`의 `0.20 - 0.15`)을 보수적으로 명시한 고정 하한이다
...
- 실질적 차이: 같은 전략의 paired 정상 완주 차이 `0.05` 이상
