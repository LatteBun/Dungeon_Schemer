...
1. 새 배경은 기존 `.is-themed` 장면용 어둡기·blur 처리 경로와 구분한다.
2. 새 배경에 blur를 추가하지 않는다. 양피지의 지도 질감은 읽을 수 있어야 한다.
3. `map_atmosphere_ruins_props.png`는 거미굴 양피지에서 숨긴다.
...
필요하면 기존 테스트 파일 안에 테마별 background source와 class 계약을 추가한다.
...
1. `themeId="spider"`이면 새 U4 양피지 PNG를 사용한다.
2. `themeId="desert"`이면 기존 desert entry 장면을 사용한다.
3. `themeId="graveyard"`이면 기존 graveyard entry 장면을 사용한다.
4. `themeId`가 없으면 기존 U4 기본 배경을 사용한다.

### 레이어

5. 거미굴 양피지에서는 기존 ruins atmosphere가 중복 렌더링되지 않는다.
6. corridor와 room layer의 z-index 관계가 유지된다.
7. 새 배경이 selectable button의 pointer/keyboard interaction을 가로채지 않는다.

### 기존 동작 회귀

8. selectable room의 mouse click이 유지된다.
9. Tab / Enter / Space 선택이 유지된다.
10. 좌우 방향키 이동이 유지된다.
11. `aria-pressed`와 `focus-visible`이 유지된다.
12. selected/current/visited/inactive 판정이 바뀌지 않는다.
