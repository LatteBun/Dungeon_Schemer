...
9. 패널의 보이는 제목은 없애고 BGM·효과음·전투 속도를 같은 높이·같은 우측 값
   칸으로 정렬한다. 업적 기록은 구분선 아래의 독립 CTA로 둔다.
10. 전투 속도는 루트 Provider의 메모리 상태이며 route 전환에는 남고 새로고침에는 `×1`로 돌아간다.
11. 공통 BGM은 승인된 `어두운 길드의 밤 1B` 방향을 바탕으로 더 자연스러운
    중세 현악 질감, 절제된 선율 변주, 낮은 북과 공간감을 함께 사용한다.
...
| 파일 | 길이 | 채널 | 용도 |
| --- | ---: | --- | --- |
| `dungeon-schemer-guild-loop.wav` | 64초 | stereo | 공통 BGM (`어두운 길드의 밤 1B`) |
| `ui-select.wav` | 0.12~0.18초 | mono | 일반 선택 |
| `ui-menu.wav` | 0.18~0.28초 | mono | 메뉴·토글 |

공통 sample rate는 44,100Hz, PCM은 signed 16-bit little-endian으로 한다. BGM은
60 BPM 16마디를 한 주기로 삼아 첫 sample과 마지막 감쇠가 자연스럽게 이어지게
만든다. 승인된 25초 `1B` 미리듣기의 방향을 64초 loop로 확장한다. 구성은 숨결이
섞인 낮은 bowed drone, 류트·덜시머를 연상시키는 유기적인 pluck, 반복마다 한두
음을 바꾸는 짧은 선율, 낮은 프레임 드럼, 드물게 들어오는 피리와 약한 공간
잔향이다. 보컬과 선명한 전투 리듬, 주의를 빼앗는 고음 반복은 쓰지 않는다.
...
상단 세 설정 행은 같은 높이와 같은 2열 grid를 쓰고 우측 값 칸도 같은 폭으로
고정한다. 그 아래에는 낮은 금속 구분선을 두고 `업적 기록`을 패널 폭을 채우는
독립 CTA로 둔다. CTA는 설정 행보다 과도하게 높거나 넓어 보이지 않되, 설정값이
아니라는 정보 계층을 분명히 한다. 전투 속도는 ON/OFF가 아니므로 일반 button으로
두고 현재 `×1` 또는 `×2`를 텍스트와 접근성 이름으로 알린다.
...
- 상단 세 설정 행의 같은 높이·우측 값 칸 폭과 구분선 아래 독립 업적 CTA
...
- BGM이 승인된 `어두운 길드의 밤 1B`의 질감·선율·저음 무게를 유지함
