...
- 성직자가 포함된 파티에는 눈에 보이는 생존 강점이 생긴다.
- 플레이어가 전투 버튼을 직접 누르지 않아도 직업 조합의 의미가 커진다.
- 치유가 공격 행동을 대신하므로 공짜 생존 보너스가 되지 않는다.
- 원정 전체 최대 회복량을 10 HP로 제한해 경로 길이와 전투 라운드 수에 따라
  회복량이 무한히 늘지 않게 한다.
- 일반 몬스터전부터 보스전까지 남은 사용 횟수를 하나의 원정 자원으로 이어 간다.

이 기능은 플레이어를 전투원으로 바꾸지 않는다. 성직자의 행동은 규칙 계층이
...
- 치유는 원정당 2회, 한 전투당 최대 1회만 사용할 수 있다.
- 생존자 중 HP가 최대 HP의 50% 이하인 인원이 있을 때만 발동한다.
- 대상은 HP 비율이 가장 낮은 생존자이며 성직자 자신도 포함한다.
- 회복량은 고정 5 HP이고 최대 HP를 넘지 않는다.
- 일반 몬스터전에서 남은 횟수가 보스전까지 이어지고, 새 원정에서는 2회로
  초기화된다.
- 공격과 치유를 서로 다른 전투 행동 기록으로 남겨 U5가 올바른 수치·동작·문장을
  재생한다.
- 같은 캠페인 시드와 같은 입력 상태는 같은 대상, 같은 행동 순서, 같은 최종 상태를
  만든다.
- 성직자가 없거나 치유 조건이 성립하지 않는 전투는 기존 공격·대상 RNG·최종 HP를
  그대로 유지한다.
...
| 조작 방식 | 규칙 계층이 자동 판정. 플레이어 직접 조작 없음 |
| 원정당 사용 횟수 | 2회 |
| 전투당 사용 횟수 | 최대 1회 |
| 발동 시점 | 성직자의 파티 행동 차례 |
...
- 원정 횟수가 1 이상 남았다.
- 현재 전투에서 아직 허용 횟수를 사용하지 않았다.
- 조건을 만족하는 생존 대상이 있다.
- 실제 회복량이 1 이상이다.

성직자가 행동 전에 사망하거나, 전투를 회피했거나, 모든 생존자가 50%를 초과하면
횟수를 소비하지 않는다. 전투당 제한은 전투가 끝나면 사라지지만 원정 잔여 횟수는
다음 전투로 이어진다.
...
export interface ClassDef {
  ...
  readonly battleAbility?: ClassBattleAbilityDef;
}
```
...
- `name`은 비어 있지 않다.
- 회복량, 원정 횟수, 전투당 횟수는 양의 안전한 정수다.
- 전투당 횟수는 원정 횟수 이하이다.
- 발동 백분율은 1 이상 100 이하의 안전한 정수다.
- 능력 정의가 없는 직업은 기존 동작을 그대로 사용한다.

잘못된 정의를 5·2·1·50으로 조용히 대체하지 않는다. 콘텐츠 검증에서
...
- 성직자가 사망해도 원정이 끝날 때까지 키는 보존한다. 사망은 잔여 횟수를 다른
  캐릭터에게 넘기지 않는다.

`createExpeditionForOffer`가 계약 파티와 현재 직업 정의를 읽어 초기 맵을 만든다.
성직자는 2로 시작한다. `START_EXPEDITION`은 파티·직업 정의·잔여 횟수의 키와 범위를
함께 검증한다.
...
→ 이번 전투 사용 한도를 넘지 않았는가
→ 50% 이하 생존 대상이 있는가
  → 있으면 가장 낮은 HP 비율 대상에게 치유, 공격 생략, continue
  → 없으면 기존 공격
→ 적이 모두 쓰러졌으면 승리
...
않는다. 생산 파티는 서로 다른 직업 3인이므로 성직자가 둘 이상 편성되지 않지만,
전투 엔진은 테스트 fixture와 향후 콘텐츠를 위해 능력 보유자별 전투 한도를 독립적으로
관리한다.

### 6.2 수정치와 치유

다음 수정치는 공격 피해에만 적용하고 고정 치유량에는 적용하지 않는다.
...
치명타, 빗나감, 임의 범위가 없고 항상 실제 1~5 HP를 회복한다.

공격을 포기하는 기회비용은 그대로 남는다. 공격 배율이 높은 상황에서는 포기한
공격의 가치도 커지지만 치유량은 5로 유지된다.
...
- `healing`은 실제 증가량이며 1 이상 능력의 `healAmount` 이하다.
...
사용하지 않았다면 보스전에서도 전투당 최대 1회만 사용하고 원정 잔여 1회를 남길 수
있다. 원정이 보스전으로 끝나므로 남은 횟수는 정산 효과로 바뀌지 않고 폐기된다.

### 8.3 집계 기록

`ExpeditionOutcome.hpChanges`와 파티 카드의 전후 HP는 전투 전체의 최종값을 계속
사용한다. 개별 치유의 주체·대상·실제 회복량은 `BattleResolution.actions`가 단일
근거다. 정산의 기존 원인 문자열 구조는 바꾸지 않으며 HP가 증가한 경우에도
`HP 이전 → 이후`를 그대로 표시한다. 치유 횟수는 명성·골드·신뢰 원인으로
승격하지 않는다.

## 9. 화면과 전투 재생

화면은 치유 여부와 회복량을 다시 판단하지 않는다. `BattleActionRecord`와
전투별 능력 상태만 재생한다.

### 9.1 재생 프레임

기존 `idle → attack → impact → settle → complete` 단계는 유지한다. 프레임에는
현재 행동 종류를 추가하고 공격 피해와 치유량을 구분한다.
...
중요한 직업 강점과 남은 자원을 숨기지 않는다. 공용 파티 카드 뷰에 선택적인
능력 상태를 추가하되 새 패널이나 새 행 높이를 만들지 않는다.
...
- U4 지도·원정 상태: 현재 원정 잔여 횟수를 `치유 2/2`, `치유 1/2`, `치유 0/2`로
  표시한다.
- U5 전투 재생: 해당 전투 시작 시 잔여 횟수에서 시작해 치유 `settle` 프레임에
  1회 감소한다.
- U6 정산·엔딩: 표시하지 않는다. 원정 뒤 유지되는 자원이 아니기 때문이다.

U5는 이미 전투가 끝난 최종 원정 상태를 미리 노출하지 않는다. 현재 전투의
`BattleResolution.party`에 남은 최종 횟수와 해당 전투의 치유 행동 수로 전투 시작
횟수를 복원하고, 재생 프레임에 맞춰 감소시킨다.
...
| `lib/content/classes.ts` | 성직자 `치유 기도` 콘텐츠 수치 |
...
| 공용 파티 카드와 U3/U4/U5 adapter | 화면별 능력 상태 표시와 미래 상태 비노출 |
| `lib/backtest/*` | 성직자 포함 여부와 치유량·횟수의 측정 |
...
2. 성직자만 `emergencyHeal`을 가지며 값이 5·2·1·50이다.
3. 능력 정의의 빈 이름, 0·음수·비정수 값, 100 초과 발동률을 거부한다.
...
8. 치유는 실제 5 또는 최대 HP까지의 부족분만 회복한다.
9. 치유한 차례에는 공격 행동이 함께 기록되지 않는다.
10. 한 전투에서 두 번째 치유 기회가 생겨도 공격하며, 다음 전투에서는 원정 횟수가
    남아 있으면 다시 한 번 치유한다.
11. 잔여 횟수 0이면 항상 공격한다.
12. 공격·조언 압력·상인·보스 정보 배율이 치유량 5를 바꾸지 않는다.
13. 치유 대상 선택이 RNG를 소비하지 않는다.
14. 능력이 없는 전투와 능력이 있으나 미발동한 전투는 변경 전 fixture와 같은 공격
    순서·대상·피해·최종 HP를 낸다.
15. 여러 능력 보유자 fixture에서는 전투당 한도를 각자 독립적으로 지킨다.

### 12.3 원정과 어댑터

16. 성직자 포함 원정은 2회로 시작하고 미포함 원정의 잔여 맵은 비어 있다.
17. 일반전 사용 뒤 1회가 다음 일반전과 보스전 입력에 이어진다.
18. 전투 회피는 횟수를 소비하지 않는다.
19. 일반전 두 번 사용 뒤 보스전에서는 치유하지 않는다.
20. 새 원정과 재도전은 다시 2회로 시작한다.
21. 정산과 캠페인 풀에는 잔여 횟수가 남지 않는다.
22. 누락·초과·파티 밖 ID·능력 없는 캐릭터의 잔여 키를 거부한다.
23. 보스 정보 큐가 치유 행동에 붙지 않고 기존 공격 행동에는 같은 위치로 붙는다.

### 12.4 U3·U4·U5

24. U3 계약 상세는 성직자에게 `치유 2회`를 보여주고 다른 직업에는 배지를 만들지
    않는다.
25. U4는 원정 상태의 현재 잔여 횟수를 표시한다.
26. U5 치유 프레임은 `+N`을 보이고 HP가 `settle`에서 증가한다.
27. 치유 대상은 피격 흔들림을 쓰지 않고 성직자는 적에게 돌진하지 않는다.
28. 치유 문장과 스크린 리더 안내가 공격·피해 표현을 사용하지 않는다.
29. U5 우측 카드는 전투 최종 잔여 횟수를 미리 보이지 않고 치유 settle에 맞춰
    감소한다.
30. 다시 보기는 해당 전투 시작 횟수, 건너뛰기는 최종 횟수를 보여준다.
31. 공격·사망·보스 정보·전투 속도 기존 재생 회귀가 통과한다.
32. FHD 고정 캔버스에서 능력 배지가 파티 카드 높이·3열 배치·스크롤을 늘리지 않는다.

### 12.5 통합과 재현성

33. 같은 캠페인 시드와 같은 선택 순서는 치유 대상·행동·잔여 횟수까지 같은 전체
    원정을 만든다.
34. Store 전체 순회, 캠페인 재현성, 정산, 통계, 이력 테스트가 새 행동 유니온을
    소비한다.
35. lint, typecheck, unit test, build, 캠페인 브라우저 smoke가 통과한다.
...
- 전투당 치유 사용 횟수 분포 0·1
- 총 치유 행동 수, 실제 총 회복량, 초과 회복으로 잘린 양
...
- 한 전투에서 같은 성직자가 1회를 초과해 치유함
- 실제 회복량이 1~5를 벗어나거나 최대 HP를 초과함
- 사망자를 치유하거나 치유와 공격을 같은 차례에 수행함
- 잔여 횟수가 증가하거나 전투 행동 수와 감소량이 다름
- 능력 미보유·미발동 기준 fixture의 결과가 달라짐
...
나오면 5·2·1·50이나 보스·몬스터·휴식·다른 직업 수치를 자동으로 바꾸지 않는다.
...
- `docs/systems/CHARACTERS_AND_TRUST.md`: 성직자의 `치유 기도`, 원정당 2회,
  전투당 1회, 50% 이하 최저 비율 대상, 고정 5 회복과 공격 포기 규칙
...
- 한 원정의 최대 회복량은 10 HP이며 한 전투 최대 회복량은 5 HP다.
- 치유는 반드시 공격 한 번을 대신하고 부활이나 전투 밖 회복을 만들지 않는다.
