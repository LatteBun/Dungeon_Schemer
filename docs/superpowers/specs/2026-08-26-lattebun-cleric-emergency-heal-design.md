...
- 상태: 사용자 설계 승인 완료, 구현 계획 작성 및 구현 완료
...
- 한 성직자의 원정 전체 최대 회복량을 10 HP로 제한해 경로 길이와 라운드 수에
  따라 회복량이 무한히 늘지 않게 한다.
...
관리한다. 본 설계의 원정 최대 10 HP·전투 최대 5 HP는 현재 생산 파티의 성직자 한
명에 대한 한도다.
...
10. 앞선 파티원이 마지막 적을 쓰러뜨리면 뒤의 성직자가 치유하거나 횟수를 쓰지
    않는다.
11. 한 전투에서 두 번째 치유 기회가 생겨도 공격하며 다음 전투에서는 원정 횟수가
    남아 있으면 다시 한 번 치유한다.
12. 잔여 횟수 0이면 항상 공격한다.
13. 공격·조언 압력·상인·보스 정보 배율이 치유량 5를 바꾸지 않는다.
14. 치유 대상 선택이 RNG를 소비하지 않는다.
15. 능력이 없는 전투와 능력이 있으나 미발동한 전투는 변경 전 fixture와 같은 공격
    순서·대상·피해·최종 HP를 낸다.
16. 여러 능력 보유자 fixture에서는 전투당 한도를 각자 독립적으로 지킨다.
...
38. lint, typecheck, unit test, build, 캠페인 브라우저 smoke가 통과한다.
...
- 현재 생산 파티에서 한 성직자의 원정 최대 회복량은 10 HP, 전투 최대 회복량은
  5 HP다.
