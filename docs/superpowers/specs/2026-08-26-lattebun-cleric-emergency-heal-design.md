...
| `lib/backtest/*` | 성직자 포함 여부와 치유량·횟수 측정 |
...
31. 다시 보기는 해당 전투 시작 횟수, 건너뛰기는 최종 횟수를 보여준다.
32. 공격·사망·보스 정보·전투 속도 기존 재생 회귀가 통과한다.
33. FHD 고정 캔버스에서 능력 배지가 파티 카드 높이·3열 배치·스크롤을 늘리지 않는다.

### 12.5 통합과 재현성

34. 같은 캠페인 시드와 같은 선택 순서는 치유 대상·행동·잔여 횟수까지 같은 전체
    원정을 만든다.
35. Store 전체 순회, 캠페인 재현성, 정산, 통계, 이력 테스트가 새 행동 유니온을
    소비한다.
36. lint, typecheck, unit test, build, 캠페인 브라우저 smoke가 통과한다.
