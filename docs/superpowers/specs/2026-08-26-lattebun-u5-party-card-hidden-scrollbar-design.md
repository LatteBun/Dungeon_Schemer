...
- 기준 브랜치: `main` (`bc07a750`, PR #190 병합 지점)
- 관련 공식 문서:
...
- 스크린 리더가 이름, 총 변화와 원인별 기록을 기존 순서대로 읽을 수 있어야 한다.
- 카드 뒤집기 버튼의 `aria-pressed`, `aria-label`, `focus-visible` 계약은 유지한다.
- 새 포커스 가능한 내부 영역이나 키보드 이벤트 핸들러를 추가하지 않는다.
- 이번 설계는 기존 카드 내부 스크롤의 키보드 조작 구조를 재설계하지 않으며, 스크롤바 비시각화가 기존 키보드 이벤트를 가로채지 않는지만 회귀 확인한다.
...
테스트 위치는 구현 계획에서 기존 `PartyMemberCard.test.tsx`와 `U5FixedCanvas.test.ts`의 책임을 비교해 한 곳으로 확정한다. 새 테스트 파일은 기존 계약 테스트에 자연스러운 소유자가 없을 때만 만든다.

### 10.2 브라우저 동작 검증

1920×1080 고정 캔버스에서 실제로 넘치는 원정 기록 fixture를 사용한다.

1. 변화 기록이 충분한 파티 카드를 뒤집는다.
...
6. 카드 우측에 네이티브 스크롤 트랙과 손잡이가 보이지 않는지 화면으로 확인한다.
7. 카드 외곽 높이, 세 카드 정렬과 우측 패널 containment가 유지되는지 확인한다.
8. 카드를 다시 덮고 다른 카드를 뒤집어 카드 전환이 정상인지 확인한다.

브라우저 자동화가 네이티브 스크롤바 픽셀을 안정적으로 판정하지 못하면, 동작은 Playwright 또는 DOM 검증으로 고정하고 시각 부재는 Chromium 화면 확인으로 보완한다.
