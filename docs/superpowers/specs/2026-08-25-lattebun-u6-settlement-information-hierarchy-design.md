...
세 인물은 생존 여부와 관계없이 같은 3열 세로형 카드로 표시한다. 각 카드는 위에서부터 직사각형 초상, 이름·직업, HP·신뢰 변화, 중대 상태 순서로 읽힌다. 초상 높이는 기존 공용 파티 상태 카드의 `--party-portrait-height` 범위를 정확히 두 배로 확장한 `clamp(11rem, 17.2cqw, 23rem)`을 사용하고 정산 카드 폭을 전부 채운다. 이름과 상태 수치는 기존 가로 행보다 크게 표시하며, 신뢰 변화는 HP 변화와 같은 글자 크기·굵기·전후 화살표 서식을 사용한다. 사망·중상·정체 발각은 색상과 텍스트를 함께 쓰는 카드 폭의 큰 하단 배지로 강조한다.
