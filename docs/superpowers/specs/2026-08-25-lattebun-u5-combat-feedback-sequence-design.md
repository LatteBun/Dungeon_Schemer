...
- 결과 단계에서 HP가 전투 전 값으로 되돌아 보이지 않으며, 변화량 유무와 무관하게
  파티 카드 외곽 크기를 동일하게 유지한다.
...
- HP settle: 수치와 막대가 replay frame 값으로 바뀌고 해당 카드는 짧게
  흔들리며 예약된 변화량 슬롯에 `HP −N`을 함께 표시한다. 이 output은 HP 통계
  grid에 조건부 행을 만들지 않는다.
...
`battleReplay`가 있는 전투에서는 `progress.party`가 전투 전 fixture 또는 오래된
화면 projection일 수 있다. 따라서 `battle` 이후의 `finalHp` 인자로
`progress.party[].hp`를 넘기지 않고, 같은 member id의 replay participant
`finalHp`를 먼저 선택한다. participant가 없을 때만 party view HP로 fallback한다.
이 우선순위는 일반전과 보스전에 동일하게 적용한다.
...
- 변화량은 다음 화면으로 이동하기 전까지 유지하며 다시보기 중에도 사라지거나
  재애니메이션하지 않는다.
- `.party-card__settled-results` 결과 슬롯은 U5의 모든 파티 카드 앞면에 항상 렌더링하고
  결과 badge 한 줄의 최소 높이를 기본으로 확보한다. 변화량이 없는 카드는 빈 슬롯만
  유지하며 `output`, 테두리, 안내 문구는 만들지 않는다. 따라서 결과가 있는 카드와
  없는 카드의 외곽 높이는 같다.
- 전투 중 transient HP·신뢰 effect도 이 슬롯의 자식으로 렌더링한다. transient
  effect와 완료 변화량은 phase상 동시에 나타나지 않으며, effect의 mount·unmount는
  슬롯이나 카드의 높이를 바꾸지 않는다.
...
- 피격 effect가 나타나는 frame과 사라진 frame의 우측 카드 높이가 같다.
- HP 0 settle frame에서만 카드가 사망 상태로 바뀐다.
- 자연 종료와 건너뛰기가 같은 final HP와 후속 phase에 도달한다.
- 전투 마지막 frame에서 표시한 HP와 `postBattleHp`, `postBattleDialogue`,
  `postBattleTrust`, `complete`의 우측 카드 HP가 같고, 단계 전환에서 증가하지 않는다.
...
- transient effect output은 예약된 결과 wrapper 안에 있고 HP·신뢰 통계 grid의
  추가 행으로 렌더링되지 않는다.
- `반응 확인` 뒤 HP·신뢰 변화량이 카드 앞면에 동시에 남고 변화가 없는 항목은
  만들지 않는다. 결과 wrapper는 모든 카드에 존재하고 같은 최소 높이를 가져,
  결과 유무로 카드 외곽 크기가 달라지지 않는다. U3·U4처럼 완료 변화량을 쓰지
  않는 화면은 빈 결과 슬롯을 요청하지 않는다.
...
4. 피격 직전·`HP −N` 표시 중·표시 후 세 파티 카드의 외곽 높이가 변하지 않는다.
5. 전투 complete 뒤 HP 강조, 사후 대사, `반응 확인`, 신뢰 변화가 순서대로
   나온다.
6. 모든 피드백이 끝난 뒤에만 지도·정산 CTA가 활성화된다.
7. 다시 보기에서는 중앙 장면만 되감기고 우측 카드는 최종 HP·신뢰·변화량을
   유지한다.
8. 보스전 마지막 frame과 `반응 확인` 전후의 우측 카드 HP가 동일하며, 세 파티
   카드의 외곽 높이가 변화량 유무와 관계없이 같다.
9. 1920×1080, 2560×1440, 1440×900, 1280×1024에서 대화 리본, 파티 카드,
   변화량과 CTA가 겹치거나 잘리지 않는다.
10. 네 viewport에서 스크롤, 이미지 왜곡, 콘솔 오류가 없다.
...
- 보스전 결과 단계에서 우측 카드 HP가 전투 마지막 frame보다 증가하지 않는다.
- 변화량이 없는 카드도 결과 badge 한 줄의 기본 높이를 확보해 모든 카드 외곽
  크기가 같다.
- 피격 `HP ±N`의 등장과 제거가 우측 카드 높이를 바꾸지 않는다.
