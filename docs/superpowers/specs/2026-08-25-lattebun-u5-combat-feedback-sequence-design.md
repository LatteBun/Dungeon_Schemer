...
- 신뢰 확인: 해당 카드의 신뢰 수치와 막대가 바뀌고 `신뢰 −N` 또는
  `신뢰 +N`을 표시한다.
- 증감은 색만으로 전달하지 않는다. 라벨·부호 있는 변화량·전후 수치를 함께
  제공한다.
...
- 장면과 우측 카드 HP는 initial frame으로 함께 돌아가 다시 동기화된다.
- 이미 확인한 신뢰는 최종값을 유지한다.
...
1. 전투 중 HP: `currentFrame.hpByParticipantId[member.id]`.
2. 전투 전 HP: replay participant의 `initialHp`.
3. 전투 complete 이후 HP: replay participant의 `finalHp`.
4. 사후 확인 전 신뢰: 현재 검증 묶음에서 첫 변화의 `before`.
5. 사후 확인 뒤 신뢰: 마지막 변화의 `after`.
6. 현재 결과에 해당 값이 없으면 기존 party view 값.
...
- 다시 보기는 HP만 되감고 신뢰·사후 대사를 재적용하지 않는다.
...
6. 다시 보기에서 HP는 장면과 카드가 함께 되감기고 신뢰는 중복 적용되지 않는다.
...
- 장면과 우측 파티 카드 HP가 같은 replay frame에서 동시에 감소한다.
