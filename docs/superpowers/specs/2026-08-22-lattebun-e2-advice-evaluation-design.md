...
E2는 사건 풀 전체의 배치, 단서 연계, merchant 결제·효과 적용, 보스 전투 계산, 화면 구현을 소유하지 않는다.
...
| E3 | 일반 사건 콘텐츠 선택, 실제 방문 사건 ID 중복 방지, 단서 누적, 약한·강한 연계, merchant 결제·효과, 방문 시 사건 물질화 |
...
`accepted`가 0명이면 조언은 실행되지 않고 `event.defaultResultText`와 사건의 기본 결과를 사용한다.
...
E3가 강한 연계 Depth를 고를 때 `BossInfoDepthPlan`의 예약 Depth를 제외한다.
...
예약 조건을 만족할 수 없으면 보장 수를 줄이지 않고 `RuleError("INVALID_GENERATION", ...)`를 반환한다.
...
- 보스 정보 Depth 예약 수나 E3 강한 연계와의 비충돌 계약을 만족할 수 없음
- 한 사건의 조언 3개 불변식이 깨짐
...
7. 한 명이라도 수용하면 한 번 실행하고, 아무도 수용하지 않으면 기본 결과를 사용한다.
