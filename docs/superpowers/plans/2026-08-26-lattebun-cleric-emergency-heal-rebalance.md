...
```bash
rg -n "healAmount|maxUsesPerBattle|고정 5 HP|전투당 최대 1회|전투당 1회" lib components \
  --glob '!**/*.test.ts' --glob '!**/*.test.tsx' \
  --glob '!**/*.spec.ts' --glob '!**/*.spec.tsx'
rg -n "healTargetMaxHpPercent|25%|전투당.*제한" lib components docs
```

Expected: 첫 명령은 production source만 검사하며 결과가 없다(`rg` exit status 1). 의도적으로 이전 계약을 거부하는 테스트와 역사 문서를 이 검사에서 제외한다. 두 번째 명령은 타입·콘텐츠·spec·공식 문서와 관련 테스트를 찾는다.
